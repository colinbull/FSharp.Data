...
type HtmlTable<'RowType> internal (name:string, headers:string[] option, values:'RowType[], data:string[][], html:HtmlNode) =
    ...
    static member Create(rowConverter:Func<string[],'RowType>, doc:HtmlDocument, id:string, hasHeaders:bool) =
        ...
        let headers, rows = 
            if hasHeaders then
                ...
                None, table.Rows
        HtmlTable<_>(table.Name, headers, Array.map rowConverter.Invoke rows, rows, table.Html)
